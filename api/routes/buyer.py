import sys
import os
sys.path.append(os.path.dirname(os.path.dirname(os.path.dirname(__file__))))

from fastapi import APIRouter, Depends, HTTPException, Query
from sqlalchemy.orm import Session
from typing import List, Optional
from datetime import datetime

from database.connection import get_db
from database.models import Buyer
from schemas.buyer import BuyerCreate, BuyerUpdate, BuyerResponse
from utils.storage import save_to_drive, update_drive_file, delete_from_drive, get_all_files, BUYER_DRIVE

router = APIRouter()

@router.post("", response_model=BuyerResponse, status_code=201)
def create_buyer(buyer: BuyerCreate, db: Session = Depends(get_db)):
    """Create new buyer"""
    
    db_buyer = Buyer(**buyer.model_dump())
    db.add(db_buyer)
    db.commit()
    db.refresh(db_buyer)
    
    # Save to drive
    buyer_dict = {
        'id': db_buyer.id,
        'name': db_buyer.name,
        'company_name': db_buyer.company_name,
        'email': db_buyer.email,
        'phone': db_buyer.phone,
        'address': db_buyer.address,
        'city': db_buyer.city,
        'state': db_buyer.state,
        'country': db_buyer.country,
        'gst_number': db_buyer.gst_number,
        'payment_terms': db_buyer.payment_terms,
        'is_active': db_buyer.is_active,
        'created_at': str(db_buyer.created_at),
        'updated_at': str(db_buyer.updated_at)
    }
    save_to_drive(BUYER_DRIVE, f"buyer_{db_buyer.id}", buyer_dict)
    
    return db_buyer

@router.get("", response_model=List[BuyerResponse])
def get_buyers(
    skip: int = Query(0, ge=0),
    limit: int = Query(10, ge=1, le=100),
    country: Optional[str] = None,
    city: Optional[str] = None,
    terms: Optional[str] = None,
    db: Session = Depends(get_db)
):
    """Get all buyers with filters"""
    
    query = db.query(Buyer)
    
    if country:
        query = query.filter(Buyer.country.ilike(f"%{country}%"))
    
    if city:
        query = query.filter(Buyer.city.ilike(f"%{city}%"))
    
    if terms:
        if terms.upper() not in ["FOB", "CIF"]:
            raise HTTPException(status_code=400, detail='Terms must be "FOB" or "CIF"')
        query = query.filter(Buyer.payment_terms == terms.upper())
    
    return query.offset(skip).limit(limit).all()

@router.get("/{buyer_id}", response_model=BuyerResponse)
def get_buyer(buyer_id: int, db: Session = Depends(get_db)):
    """Get specific buyer"""
    
    db_buyer = db.query(Buyer).filter(Buyer.id == buyer_id).first()
    if not db_buyer:
        raise HTTPException(status_code=404, detail="Buyer not found")
    return db_buyer

@router.put("/{buyer_id}", response_model=BuyerResponse)
def update_buyer(buyer_id: int, buyer: BuyerUpdate, db: Session = Depends(get_db)):
    """Update buyer"""
    
    db_buyer = db.query(Buyer).filter(Buyer.id == buyer_id).first()
    if not db_buyer:
        raise HTTPException(status_code=404, detail="Buyer not found")
    
    # Update fields
    update_data = buyer.model_dump(exclude_unset=True)
    for field, value in update_data.items():
        setattr(db_buyer, field, value)
    
    db_buyer.updated_at = datetime.utcnow()
    db.commit()
    db.refresh(db_buyer)
    
    # Update drive file
    buyer_dict = {
        'id': db_buyer.id,
        'name': db_buyer.name,
        'company_name': db_buyer.company_name,
        'email': db_buyer.email,
        'phone': db_buyer.phone,
        'address': db_buyer.address,
        'city': db_buyer.city,
        'state': db_buyer.state,
        'country': db_buyer.country,
        'gst_number': db_buyer.gst_number,
        'payment_terms': db_buyer.payment_terms,
        'is_active': db_buyer.is_active,
        'created_at': str(db_buyer.created_at),
        'updated_at': str(db_buyer.updated_at)
    }
    update_drive_file(BUYER_DRIVE, f"buyer_{buyer_id}", buyer_dict)
    
    return db_buyer

@router.delete("/{buyer_id}", status_code=204)
def delete_buyer(buyer_id: int, db: Session = Depends(get_db)):
    """Delete buyer"""
    
    db_buyer = db.query(Buyer).filter(Buyer.id == buyer_id).first()
    if not db_buyer:
        raise HTTPException(status_code=404, detail="Buyer not found")
    
    db.delete(db_buyer)
    db.commit()
    delete_from_drive(BUYER_DRIVE, f"buyer_{buyer_id}")
    
    return None

@router.get("/drive-info", include_in_schema=False)
def get_drive_info():
    """Get drive information"""
    import os
    files = get_all_files(BUYER_DRIVE)
    
    return {
        "status": "success",
        "drive_path": BUYER_DRIVE,
        "folder_exists": os.path.exists(BUYER_DRIVE),
        "is_writable": os.access(BUYER_DRIVE, os.W_OK) if os.path.exists(BUYER_DRIVE) else False,
        "total_files": len(files),
        "files": files
    }